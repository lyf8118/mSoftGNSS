...
% Performs code and carrier tracking for E6 signals of all channels.
%
...
codePeriods = settings.msToProcess;     % For Galileo one E6 code is one ms
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        % Get a vector with the E6B tiered code sampled 1x/chip
        E6BCode = generateE6Bcode(channel(channelNr).PRN);  
        % Then make it possible to do early and late versions
        E6BCode = [E6BCode(settings.codeLength) E6BCode E6BCode(1)]; %#ok<AGROW>

                    % Get a vector with the E6C primary code sampled 1x/chip
            E6cCode = generateE6Ccode(channel(channelNr).PRN,1);
			% Then make it possible to do early and late versions
            E6cCode = [E6cCode(settings.codeLength) E6cCode E6cCode(1)];

        E6CodeTable = [E6BCode E6cCode];
        ...
        for loopCnt =  1:codePeriods
            ...
            if settings.correlatorType == 0 % Using Matlab correlator
                correValues = corrMatlabSerialQPSK(settings, rawSignal, ...
                    E6CodeTable, remCarrPhase, carrPhaseStep, remCodePhase,...
                    codePhaseStep);
            ...
            % chaannel signals
            if (settings.pilotTRKflag == 1)
                % Pilot channel(E6C) carrier phase is pi/2 rad ahead of the
                % data channel (E6B) carrier phase.
                % phase pi/2 back to the E6C phase.
