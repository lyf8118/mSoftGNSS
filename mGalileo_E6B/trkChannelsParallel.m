...
% The absolute sample in the record of the E6 code start:
...
if (settings.pilotTRKflag == 1)
    ...
    trackResults.E6_CNo  = zeros(1,floor(settings.msToProcess/settings.CNoInterval));
end
...
% The absolute sample in the record of the E6 code start for all channels.
...
% Generate the E6 code table for acquired signals
for channelNr = 1:channelCnt
    % Get a vector with the E5b data code sampled 1x/chip
    E6bCode = generateE6Bcode(channel(channelNr).PRN);
    % Then make it possible to do early and late versions
    E6bCodeTable(channelNr,:)  = [E6bCode(end) E6bCode E6bCode(1)]; %#ok<AGROW>
    % Get a vector with the E5b pilot code sampled 1x/chip
    E6cCode = generateE6Ccode(channel(channelNr).PRN,1);
    % Then make it possible to do early and late versions
    E6cCodeTable(channelNr,:)  = [E6cCode(end) E6cCode E6cCode(1)]; %#ok<AGROW>
end
E6CodeTable = [E6bCodeTable E6cCodeTable];
if settings.correlatorType == 1
    % SIMD backend expects one int32 code table shared by all active channels.
    E6CodeTable = int32(E6CodeTable');
elseif settings.correlatorType == 2
    % GPU backend expects one int8 code table shared by all active channels.
    E6CodeTable = int8(E6CodeTable');
end

%% Initialize tracking variables ==================================
% Code periods to be processed
codePeriods = settings.msToProcess;     % For GAL one E6 code is one ms
%--- DLL variables --------------------------------------------------------
% Summation interval
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelQPSK(settings,rawSignal,E6CodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,E6CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelQPSK(settings,rawSignal,E6CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        if (rem(loopCnt,settings.CNoInterval)==0) 
            ...
            if (settings.pilotTRKflag == 1)
                ...
                trackResults(channelNr).E6_CNo(CNoCnt) = ... 
