...
for PRN = settings.acqSatelliteList
    ...
    % Generate all E6C+E6B primary codes and sample them according to the sampling freq.
    ...
    localE6CCode = gpuArray(single([E6CCodesTable, zeros(1,samplesPerCode)]));
    localE6BCode = gpuArray(single([E6BCodesTable, zeros(1,samplesPerCode)]));
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        for nonCohIndex = 1: settings.acqNonCohTime
            ...
            results = results + abs(ifft(convE6C)) + abs(ifft(convE6B));
        end
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        E6bCode = gpuArray(single(generateE6Bcode(PRN)));
        E6cCode = gpuArray(single(generateE6Ccode(PRN,1)));
        ...
        basebandData = reshape(sig200ms .* E6bCode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
        basebandPilot = reshape(sig200ms .* E6cCode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
