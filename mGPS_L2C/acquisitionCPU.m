...
% Performs cold-start acquisition for GPS L2C signals using CPU-based
% MATLAB arrays. The function searches all PRNs listed in
% settings.acqSatelliteList and stores the detected CM code phase, CL code
...
% Find number of samples per full L2C CM code period and per chip. In this
% receiver, settings.codeLength corresponds to one 20 ms CM code period.
...
% of this block, while fine frequency estimation below uses 10 CM periods
...
phasePoints = (0 : (samplesPerCode * 10-1)) * 2 * pi * ts;
...
% Code resampling index used by both CM fine acquisition and CL phase search.
% codeValueIndex maps each input sample to the corresponding local code chip.
tc = 1/(settings.codeFreqBasis);
codeValueIndex = floor(ts * (0: 10*samplesPerCode-1)/tc);
codeValueIndex = rem(codeValueIndex, settings.codeLength) + 1;

% Scratch buffer for the 75 possible L2 CL code phase positions.
...
% L2 CM code phases of detected signals.
acqResults.codePhase    = zeros(1, 32);
% Correlation peak ratios of the detected signals.
acqResults.peakMetric   = zeros(1, 32);
% L2 CL code phase index in the 75-period CL search space.
acqResults.CLCodePhase =  zeros(1, 32);
...
for PRN = settings.acqSatelliteList
    ...
    %% Coarse acquisition =========================================
    % Generate the sampled L2 CM local code for this PRN according to the
    % receiver sampling frequency.
    cmCodesTable = makeCMTable(settings,PRN);
    % Append one zero-padded CM period so the 2-period FFT correlation below
    % can cover the complete circular code-phase search range.
    localCmCode = [cmCodesTable, zeros(1,samplesPerCode)];
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        % Wipe off the carrier hypothesis and convert the first two CM code
        % periods of the input signal to the frequency domain.
        IQfreqDom = fft(sigCarr .* longSignal(1:samplesPerCode*2));
    ...
    % acquisition, move to the previous CM code start position when the peak
    % falls in the second padded period.
    if codePhaseMax >= samplesPerCode
        codePhaseMax = codePhaseMax - samplesPerCode;
    end

    %--- Find 1-chip-wide L2 CM code phase exclude range around the peak ---
    ...
    % Save CM code phase acquisition result.
    ...
    % frequency and determine the L2 CL code phase.
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % Generate one unresampled L2 CM code period.
        L2CmCode = generateCMcode(PRN,settings);

        % Resample the local CM code over 10 CM periods (200 ms).
        L2CmCode200ms = L2CmCode(codeValueIndex);

        % Extract 10 CM periods (200 ms) of incoming signal starting from the
        % detected CM code phase.
        sig200ms = longSignal(codePhaseMax:codePhaseMax + 10*samplesPerCode -1);

        % Coarse-frequency local carrier used before fine frequency estimate.
        localCarr200cm = exp(-1i * freqMax * phasePoints);

        %--- Integration for each 1 ms segment over 200 ms -----------------
        % Wipe off CM code and coarse carrier from the incoming signal.
        ...
        samplesPer1ms = floor(samplesPerCode/20) * 200;
        sumPerCode = sum(reshape(basebandSig(1:samplesPer1ms),[],200));
        ...
        % Wipe off the refined carrier over one CM period and test all 75 CL
        ...
        L2ClCode = generateCLcode(PRN,settings);

        % Search the 75 possible CL-code alignments relative to the CM period.
        for ind = 1:75
            L2ClCodeSample = L2ClCode(codeValueIndex(1:samplesPerCode) + ...
                settings.codeLength * (ind-1));
            powerArray(1,ind) = abs(sum(sig200ms(1:samplesPerCode) ...
                .* L2ClCodeSample .*sigCarr));
        end

        % Store the selected CL code phase index.
        [~,tempPhase]  = max(powerArray);
        acqResults.CLCodePhase(PRN) = tempPhase;
    else
