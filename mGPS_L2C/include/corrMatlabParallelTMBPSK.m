function correValues = corrMatlabParallelQPSK(settings, rawSignal, L2CodeTable, ...
    remCarrPhase, carrPhaseStep, remCodePhase, codePhaseStep, ...
    startIdx, chSampSize, isDataRead)
%CORRMATLABPARALLELQPSK MATLAB channel-parallel GPS L2C tracking correlator.
...
%       L2CodeTable(channel,:) = [CM code with guard chips, ...
%                                 CL code with guard chips]
%   The first half is the repeated CM data branch, and the second half is
%   the CL pilot branch. Each branch already includes one wraparound chip on
...
for channelNr = 1:channelCnt
    % Find the size of the current L2C code period in whole samples for the
    ...
    % Get the local L2C code table for the current channel. The first half
    % is the CM data branch; the second half is the CL pilot branch.
    ...
    % Define index into early CM / CL code vectors.
    ...
    % Define index into late CM / CL code vectors.
    tcode = ceil(codePhase + earlyLateSpc) + 1;
    ...
    % Define index into prompt CM / CL code vectors.
    ...
    % Now get early, late, and prompt values for the CM data branch.
    ...
    if settings.pilotTRKflag == 1
        % Now get early, late, and prompt values for the CL pilot branch.
