...
% Performs GPS L2C code and carrier tracking in channel-parallel mode.
%
...
if (settings.pilotTRKflag == 1)
    ...
    trackResults.L2C_CNo  = zeros(1,tempCnt);
end
...
% Per-channel CL period index corresponding to the current CM code epoch.
CLCodePhase = [channel(1:channelCnt).CLCodePhase];
% Absolute IF sample index of each channel's next L2C code epoch.
absoluteSample = settings.skipNumberOfSamples + ...
    [channel(1:channelCnt).codePhase] - 1;

% Generate L2C CM/CL code tables for all acquired signals.
...
for loopCnt =  1:NumToProcess
    ...
    % Convert each residual phase within the current CM interval into an
    % absolute phase within the 75-period CL sequence.
    remCodePhaseL2C = remCodePhase + settings.codeLength *(CLCodePhase-1);
    ...
    if settings.correlatorType == 0 % Using MATLAB correlator
        correValues = corrMatlabParallelTMBPSK(settings,rawSignal,L2CodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,L2CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhaseL2C,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelQPSK(settings,rawSignal,L2CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhaseL2C,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        remCodePhase(channelNr) = rem(chSampSize(channelNr) * ...
            codePhaseStep(channelNr) + remCodePhaseL2C(channelNr),settings.codeLength);
        ...
        CLCodePhase(channelNr) = CLCodePhase(channelNr) + 1;
