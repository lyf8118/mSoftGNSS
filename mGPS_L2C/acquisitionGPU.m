...
% Performs cold-start acquisition for GPS L2C signals using MATLAB GPU
% arrays. The function searches all PRNs listed in settings.acqSatelliteList
% and stores the detected CM code phase, CL code phase, coarse/fine carrier
...
powerArray = zeros(1,75,'single','gpuArray');
...
for PRN = settings.acqSatelliteList
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        L2CmCode = gpuArray(single(generateCMcode(PRN,settings)));
        ...
        L2ClCode = gpuArray(single(generateCLcode(PRN,settings)));
        ...
        acqResults.CLCodePhase(PRN) = gather(tempPhase);
    else
