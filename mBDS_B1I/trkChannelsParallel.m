...
% The absolute sample in the record of the B1I/B2I code start:
...
% The absolute sample in the record of the B1I/B2I code start for all channels.
...
for channelNr = 1:channelCnt
    % Get a vector with the B1I/B2I code sampled 1x/chip
    B12Icode = generateB12Icode(channel(channelNr).PRN);
    % Then make it possible to do early and late versions
    B12IcodeTable(channelNr,:) = [B12Icode(end) B12Icode B12Icode(1)];  %#ok<AGROW>
end

if settings.correlatorType == 1
    % SIMD backend expects one int32 code table shared by all active channels.
    B12IcodeTable = int32(B12IcodeTable');
elseif settings.correlatorType == 2
    % GPU backend expects one int8 code table shared by all active channels.
    B12IcodeTable = int8(B12IcodeTable');
end
%% Initialize tracking variables ==================================
% Code periods to be processed
codePeriods = settings.msToProcess;     % For BDS one B1I/B2I code is one ms
%--- DLL variables --------------------------------------------------------
% Summation interval
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelBPSK(settings,rawSignal,B12IcodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelBPSK(settings,rawSignal,B12IcodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelBPSK(settings,rawSignal,B12IcodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
...
% Close the waitbar
close(hwb)
clear corrSIMDParallelBPSK corrGPUParallelBPSK;
