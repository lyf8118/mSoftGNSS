...
channel.codeFreq        = 0;    % Initial code NCO frequency
channel.L1OCpCodePhase       = 0;  % First L1Ocp code position 
...
for ii = 1:min([settings.numberOfChannels, sum(acqResults.carrFreq ~= 0)])
    ...
    channel(ii).codeFreq     = acqResults.codeFreq(PRNindexes(ii));
    channel(ii).L1OCpCodePhase  = acqResults.L1OCpCodePhase(PRNindexes(ii));
