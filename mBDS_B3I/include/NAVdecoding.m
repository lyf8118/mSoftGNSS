...
%NAVDECODING finds BDS B3I preambles and decodes D1 or D2 navigation data.
%Preamble candidates are verified using the expected spacing between two
%300-bit messages and the BCH check of the first word. The D1 message is
%used by MEO/IGSO satellites, while the D2 message is used by GEO
%satellites.
%
%[eph, subFrameStart,SOW] = NAVdecoding(I_P_InputBits,PRN)
%
%   Inputs:
%       I_P_InputBits   - prompt in-phase correlator output from tracking,
%                       with one value per 1 ms B3I code period.
%       PRN             - satellite PRN number. It determines whether the
%                       D1 or D2 navigation message is decoded.
%
%   Outputs:
%       subFrameStart   - one-based position of the first valid D1 subframe
%                       or D2 page in I_P_InputBits, expressed as a 1 ms
%                       tracking-period index from the start of tracking.
%                       It is set to inf if no valid preamble is detected.
%       SOW             - Second Of Week (SOW) of the first D1 subframe or
%                       D2 page in seconds. It is set to inf if navigation
...
% "Upsample" the preamble - make 20 values per one bit (D1) & 2 values per
...
availablePeriods = length(I_P_InputBits) - subFrameStart + 1;

% GEO satellites. Not modulated with NH code.
if ((1 <= PRN) && (PRN <=5 )) || ((59 <= PRN) && (PRN <= 63))
    %--- Copy 50 D2 pages from tracking output -----------------------------
    % Fifty 300-bit pages contain 15,000 bits and each bit is 2 ms.
    navBitCount = 15000;
    if availablePeriods < navBitCount * 2
        disp(['    Insufficient navigation data for PRN # ',num2str(PRN),'!']);
        subFrameStart = inf;
        return
    end
    navBitsSamples = I_P_InputBits(subFrameStart: ...
        subFrameStart + navBitCount * 2 - 1)';
    
    %--- Group every 2 values of bits into columns ------------------------
    % 1 bit is 2ms long. Raw bit is converted to Navigation bit.
    % 30,000 raw bits --> 15,000 navigation bits.
    navBitsSamples = reshape(navBitsSamples, ...
        2, (size(navBitsSamples, 1) / 2));
    
% MEO/IGSO satellites. Modulated with the 20-chip NH code.
elseif (6 <= PRN) && (PRN <= 58)
    %--- Copy five to seven D1 subframes from tracking output --------------
    % Up to seven subframes ensure that subframes 1, 2 and 3 can be selected
    % from one five-subframe cycle when tracking starts at subframe 2 or 3.
    frameCount = min(7, floor(availablePeriods / (300 * 20)));
    if frameCount < 5
        disp(['    Insufficient navigation data for PRN # ',num2str(PRN),'!']);
        subFrameStart = inf;
        return
    ...
    navBitsSamples = I_P_InputBits(subFrameStart: ...
        subFrameStart + navBitCount * 20 - 1)';
    
    % Demodulating NH code
    demod_bit = kron(ones(1,navBitCount), NH_code);
    ...
    % Raw tracking values are integrated into one navigation bit per column.
    navBitsSamples = reshape(navBitsSamples, ...
