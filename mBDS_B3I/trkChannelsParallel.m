...
% The absolute sample in the record of the B3I code start:
...
% The absolute sample in the record of the B3I code start for all channels.
...
for channelNr = 1:channelCnt
    % Get a vector with the B3I code sampled 1x/chip
    B3ICode = generateB3Icode(channel(channelNr).PRN);
    % Then make it possible to do early and late versions
    B3ICodeTable(channelNr,:) = [B3ICode(end) B3ICode B3ICode(1)];  %#ok<AGROW>
end

if settings.correlatorType == 1
    % SIMD backend expects one int32 code table shared by all active channels.
    B3ICodeTable = int32(B3ICodeTable');
elseif settings.correlatorType == 2
    % GPU backend expects one int8 code table shared by all active channels.
    B3ICodeTable = int8(B3ICodeTable');
end
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelBPSK(settings,rawSignal,B3ICodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelBPSK(settings,rawSignal,B3ICodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelBPSK(settings,rawSignal,B3ICodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
