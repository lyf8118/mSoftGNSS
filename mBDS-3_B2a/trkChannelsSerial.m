...
% Performs code and carrier tracking for BDS-3 B2a signals of all channels.
%
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        % appropriate sample (corresponding to code phase). The configured
        % skip and code phase are both expressed in logical samples.
        fseek(fid, (settings.skipNumberOfSamples + ...
            channel(channelNr).codePhase-1)*dataAdaptCoeff*bytesPerScalar, 'bof');

        % Get a vector with the B2a data code sampled 1x/chip
        B2aCodeD = generateB2aDataCode(channel(channelNr).PRN,settings);
        % Then make it possible to do early and late versions
        B2aCodeD = [B2aCodeD(settings.codeLength) B2aCodeD B2aCodeD(1)]; 

        % Get a vector with the B2a pilot code sampled 1x/chip
        B2aCodeP = generateB2aPilotCode(channel(channelNr).PRN,settings);
        % Then make it possible to do early and late versions
        B2aCodeP = [B2aCodeP(settings.codeLength) B2aCodeP B2aCodeP(1)];

        B2aCodeTable = [B2aCodeD B2aCodeP];
