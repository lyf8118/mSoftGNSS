...
% B2a code phases of detected signals
...
for PRN = settings.acqSatelliteList
    %% Coarse acquisition =========================================
    % Generate B2a data and pilot codes and sample them
    ...
    B2aDataCode2ms = gpuArray(single([B2aDataTable, zeros(1,samplesPerCode)]));
    B2aPilotCode2ms = gpuArray(single([B2aPilotTable, zeros(1,samplesPerCode)]));
    
    %--- Perform DFT of B2a data and pilot codes ------------------------------------------
    B2aDataFreqDom = conj(fft(B2aDataCode2ms));
    B2aPilotFreqDom = conj(fft(B2aPilotCode2ms));
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        for nonCohIndex = 1: settings.acqNonCohTime
            ...
            results = results + abs(ifft(convB2aData)) + abs(ifft(convB2aPilot));
        end
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % B2a data and pilot codes
        B2aDataCode = gpuArray(single(generateB2aDataCode(PRN,settings)));
        B2aPilotCode = gpuArray(single(generateB2aPilotCode(PRN,settings)));
        
        % B2a codes sample index
        codeValueIndex = gpuArray(single( floor((ts * ...
            (0 : 200*samplesPerCode -1)) * settings.codeFreqBasis ) ));
        
        % Sampled data and pilot codes
        B2aDataCode200ms = B2aDataCode((rem(codeValueIndex, settings.codeLength) + 1));
        B2aPilotCode200ms = B2aPilotCode((rem(codeValueIndex, settings.codeLength) + 1));
        ...
        basebandData = reshape(sig200ms .* B2aDataCode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
        basebandPilot = reshape(sig200ms .* B2aPilotCode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
