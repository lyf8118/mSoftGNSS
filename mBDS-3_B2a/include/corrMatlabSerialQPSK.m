function correValues = corrMatlabSerialQPSK(settings, rawSignal, B2aCodeTable, ...
    remCarrPhase, carrPhaseStep, remCodePhase, codePhaseStep)

codeLen = length(B2aCodeTable)/2;
B2aCodeD = B2aCodeTable(1:codeLen);
B2aCodeP = B2aCodeTable(1+codeLen:end);
...
tcode       = (remCodePhase+earlyLateSpc) : ...
    ...
    ((blksize-1)*codePhaseStep+remCodePhase+earlyLateSpc);
tcode2      = ceil(tcode) + 1;
lateCode    = B2aCodeD(tcode2);

% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    lateCodeQ   = B2aCodeP(tcode2);
end
...
tcode2      = ceil(tcode) + 1;
promptCode  = B2aCodeD(tcode2);

% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    promptCodeQ   = B2aCodeP(tcode2);
end
