function correValues = corrMatlabParallelQPSK(settings, rawSignal, B2aCodeTable, ...
...
for channelNr = 1:channelCnt
    ...
    codeLen = length(B2aCode)/2;
    B2aCodeD = B2aCode(1:codeLen);
    if (settings.pilotTRKflag == 1)
        B2aCodeP = B2aCode(1+codeLen:end);
    end
