function code = B2a_EMS_decode(L)  
    init_table();
    ...
    % NM_EMS = 4;         % EMS LLR truncation size

    n= 96;
    [~, min_indices] = min(L, [], 2); % Minimum-cost symbol index per row
    code = min_indices - 1;          
    ...
    for iter = 1:MAX_ITER  
        ...
        c = gf(code, 6);  % GF(64) code symbols
        Ho = gf(H.', 6);  % GF(64) parity-check matrix
        % Check whether the decoded codeword satisfies the parity equations
        s = c * Ho;
