...
%Function generates B2a pilot codes for the specified PRN based on the settings
%provided in the structure "settings". The codes are digitized at the
%sampling frequency specified in the settings structure.
%One row in the "B2aPilotCodesTable" is one B2a pilot code. The row number is the PRN
%number of the B2a pilot code.
%
%B2aPilotCodesTable = makeB2aPilotTable(settings, PRN)
%
%   Inputs:
%       settings        - receiver settings
%   Outputs:
%       B2aPilotCodesTable   - an array of arrays (matrix) containing B2a pilot codes
...
%CVS record:
%$Id: makeB2aPilotTable.m,v 1.1.2.6 2006/08/14 11:38:22 dpl Exp $
...
% chip array (B2a pilot code) for the time instances of each sample.
