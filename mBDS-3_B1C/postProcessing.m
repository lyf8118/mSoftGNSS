% Script postProcessing.m processes the raw BDS-3 B1C signal from the
% specified data file using the duration configured in settings.
%
...
% the coherent integration results. After tracking, postNavigation is called.
% It calculates pseudoranges
...
if (fid > 0)
    ...
    if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
        ...
        if settings.gpuACQflag == 0
            % Use CPU acquisition
            acqResults = acquisitionCPU(fid,settings);
        elseif settings.gpuACQflag == 1
            % Use GPU acceleration for acquisition
            acqResults = acquisitionGPU(fid,settings);
        end
        
        disp(['   Elapsed time for acquisition is ', ...
            datestr(datetime("now") - startTime, 13), '.'])
    ...
    disp('   Processing is complete for this data file');
