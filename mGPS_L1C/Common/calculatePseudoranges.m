...
%       subFrameStart   - array containing the first CNAV-2 frame start
%                       index in each channel. The position is counted in
%                       tracking integration epochs from the start of
%                       tracking. The value is set to 0 if no valid frame
%                       was decoded in the channel:
%                       1 by settings.numberOfChannels
%       TOW             - Time Of Week (TOW) of the first decoded CNAV-2
%                       frame in the bit stream (in seconds)
%       currMeasSample  - current measurement sample location(measurement time)
%       localTime       - local time in the GPS time scale at measurement time
...
%       transmitTime    - transmission time of channels to be processed
...
for channelNr = channelList
    ...
    % Transmitting time (in seconds) at current measurement sample location
...
%--- Convert travel-time measurement to distance --------------------------
