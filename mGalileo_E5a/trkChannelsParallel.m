...
% The absolute sample in the record of the E5a code start:
...
if (settings.pilotTRKflag == 1)
    ...
    trackResults.E5a_CNo  = zeros(1,floor(settings.msToProcess/settings.CNoInterval));
end
...
% The absolute sample in the record of the E5a code start for all channels.
...
% Generate the E5a code table for acquired signals
for channelNr = 1:channelCnt
    % Get a vector with the E5a data code sampled 1x/chip
    E5aICode = generateE5aIcode(channel(channelNr).PRN,1);
    % Then make it possible to do early and late versions
    E5aICodeTable(channelNr,:)  = [E5aICode(end) E5aICode E5aICode(1)]; %#ok<AGROW>
    % Get a vector with the E5a pilot code sampled 1x/chip
    E5aQcode = generateE5aQcode(channel(channelNr).PRN,1);
    % Then make it possible to do early and late versions
    E5aQcodeTable(channelNr,:)  = [E5aQcode(end) E5aQcode E5aQcode(1)]; %#ok<AGROW>
end
E5aCodeTable = [E5aICodeTable E5aQcodeTable];
if settings.correlatorType == 1
    % SIMD backend expects one int32 code table shared by all active channels.
    E5aCodeTable = int32(E5aCodeTable');
elseif settings.correlatorType == 2
    % GPU backend expects one int8 code table shared by all active channels.
    E5aCodeTable = int8(E5aCodeTable');
end

%% Initialize tracking variables ==================================
% Code periods to be processed
codePeriods = settings.msToProcess;     % For GAL one E5a code is one ms
%--- DLL variables --------------------------------------------------------
% Summation interval
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelQPSK(settings,rawSignal,E5aCodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,E5aCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelQPSK(settings,rawSignal,E5aCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        % chaannel signals
        if (settings.pilotTRKflag == 1)
            % E5a pilot channel carrier phase is pi/2 rad ahead of the
        ...
        if (rem(loopCnt,settings.CNoInterval)==0) 
            ...
            if (settings.pilotTRKflag == 1)
                ...
                trackResults(channelNr).E5a_CNo(CNoCnt) = ... 
