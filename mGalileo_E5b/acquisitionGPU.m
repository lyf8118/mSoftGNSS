...
%searches for E5b signals of all satellites, which are listed in field
...
for PRN = settings.acqSatelliteList
    ...
    % Generate all E5bI+E5bQ primary codes and sample them
    ...
    E5bICode2ms = gpuArray(single([E5bICodesTable, zeros(1,samplesPerCode)]));
    E5bQCode2ms = gpuArray(single([E5bQCodesTable, zeros(1,samplesPerCode)]));
    
    %--- Perform DFT of PRN code ------------------------------------------
    E5bICodeFreqDom = conj(fft(E5bICode2ms));
    E5bQCodeFreqDom = conj(fft(E5bQCode2ms));
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        for nonCohIndex = 1: settings.acqNonCohTime
            ...
            results = results + abs(ifft(convE5bI)) + abs(ifft(convE5bQ));
        end
    ...
    %--- Find 1 chip wide E5b code phase exclude range around the peak ----
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % E5b data and pilot codes
        E5bICode = gpuArray(single(generateE5bIcode(PRN,1)));
        E5bQCode = gpuArray(single(generateE5bQcode(PRN,1)));
        ...
        E5bICode200ms = E5bICode((rem(codeValueIndex, settings.codeLength) + 1));
        E5bQCode200ms = E5bQCode((rem(codeValueIndex, settings.codeLength) + 1));
        ...
        basebandData = reshape(sig200ms .* E5bICode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
        basebandPilot = reshape(sig200ms .* E5bQCode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
