...
% The absolute sample in the record of the E5b code start:
...
% The absolute sample in the record of the E5b code start for all channels.
...
% Generate the E5b code table for acquired signals
for channelNr = 1:channelCnt
    ...
    E5bICodeTable(channelNr,:)  = [E5bICode(end) E5bICode E5bICode(1)]; %#ok<AGROW>
    % Get a vector with the E5b pilot code sampled 1x/chip
    E5bQcode = generateE5bQcode(channel(channelNr).PRN,1);
    % Then make it possible to do early and late versions
    E5bQcodeTable(channelNr,:)  = [E5bQcode(end) E5bQcode E5bQcode(1)]; %#ok<AGROW>
end
E5bCodeTable = [E5bICodeTable E5bQcodeTable];
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelQPSK(settings,rawSignal,E5bCodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,E5bCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelQPSK(settings,rawSignal,E5bCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        % chaannel signals
        if (settings.pilotTRKflag == 1)
            % E5b pilot channel carrier phase is pi/2 rad ahead of the
