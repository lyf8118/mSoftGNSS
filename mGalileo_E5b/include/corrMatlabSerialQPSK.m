function correValues = corrMatlabSerialQPSK(settings, rawSignal, E5bCodeTable, ...
    remCarrPhase, carrPhaseStep, remCodePhase, codePhaseStep)

codeLen = length(E5bCodeTable)/2;
E5bCodeD = E5bCodeTable(1:codeLen);
E5bCodeP = E5bCodeTable(1+codeLen:end);
...
tcode       = (remCodePhase+earlyLateSpc) : ...
    ...
    ((blksize-1)*codePhaseStep+remCodePhase+earlyLateSpc);
tcode2      = ceil(tcode) + 1;
lateCode    = E5bCodeD(tcode2);

% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    lateCodeQ   = E5bCodeP(tcode2);
end
...
tcode2      = ceil(tcode) + 1;
promptCode  = E5bCodeD(tcode2);

% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    promptCodeQ   = E5bCodeP(tcode2);
end
