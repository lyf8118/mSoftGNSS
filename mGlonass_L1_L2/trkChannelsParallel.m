...
codePeriods = settings.msToProcess; % For GLONASS one L1/L2 C/A code is one ms
%--- DLL variables --------------------------------------------------------
% Summation interval
...
for loopCnt =  1:codePeriods
    ...
    if (rem(loopCnt, barUprate) == 0)
        Ln = newline;
        trackingStatus=['Tracking ', int2str(channelCnt),' PRNs(K):',Ln ...
            '1st PRN(K): ', int2str(channel(1).K),Ln ...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelBPSK(settings,rawSignal,caCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        if loopCnt == codePeriods
            ...
            trackResults(channelNr).K     = channel(channelNr).K;
