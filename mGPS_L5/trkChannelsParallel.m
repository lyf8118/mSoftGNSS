...
%--------------------------------------------------------------------------
%                         CU Multi-GNSS SDR
% (C) Developed for GPS L5 SDR by Yafeng Li, Nagaraj C. Shivaramaiah
% and Dennis M. Akos.
...
if (settings.pilotTRKflag == 1)
    ...
    trackResults.L5C_CNo  = zeros(1,floor(settings.msToProcess/settings.CNoInterval));
end
...
% The absolute sample in the record of the L5 code start for all channels.
...
% Generate the L5 code table for acquired signals
for channelNr = 1:channelCnt
    % Get a vector with the L5C data code sampled 1x/chip
    L5ICode = generateL5Icode(channel(channelNr).PRN,settings);
    % Then make it possible to do early and late versions
    L5ICodeTable(channelNr,:)  = [L5ICode(end) L5ICode L5ICode(1)]; %#ok<AGROW>
    % Get a vector with the L5C pilot code sampled 1x/chip
    L5QCode = generateL5Qcode(channel(channelNr).PRN,settings);
    % Then make it possible to do early and late versions
    L5QCodeTable(channelNr,:)  = [L5QCode(end) L5QCode L5QCode(1)]; %#ok<AGROW>
end
L5CodeTable = [L5ICodeTable L5QCodeTable];
if settings.correlatorType == 1
    % SIMD backend expects one int32 code table shared by all active channels.
    L5CodeTable = int32(L5CodeTable');
elseif settings.correlatorType == 2
    % GPU backend expects one int8 code table shared by all active channels.
    L5CodeTable = int8(L5CodeTable');
end
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelQPSK(settings,rawSignal,L5CodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,L5CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelQPSK(settings,rawSignal,L5CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        % chaannel signals
        if (settings.pilotTRKflag == 1)
        % L5C pilot channel carrier phase is pi/2 rad ahead of the
        ...
        if (rem(loopCnt,settings.CNoInterval)==0) 
            ...
            if (settings.pilotTRKflag == 1)
                ...
                trackResults(channelNr).L5C_CNo(CNoCnt) = ... 
