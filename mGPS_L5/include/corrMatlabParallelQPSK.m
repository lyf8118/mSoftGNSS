function correValues = corrMatlabParallelQPSK(settings, rawSignal, L5CCodeTable, ...
...
for channelNr = 1:channelCnt
    ...
    codeLen = length(L5CCode)/2;
    L5CCodeD = L5CCode(1:codeLen);
    if (settings.pilotTRKflag == 1)
        L5CCodeP = L5CCode(1+codeLen:end);
    end
