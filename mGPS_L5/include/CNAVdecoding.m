...
% Set parameters and allocate variable space ------------------------------
...
% skipped here to avoid tracking loop transients.
...
for G1orG2 = 1:2
    ...
    for i = 1:numel(index) % For each occurrence
        ...
        if ((length(decodedBits) - index(i) +1) >=300 )
            ...
            %--- To do CRC-24Q check of current message -----------------------------
            ...
            if (~frmError) 
                ...
                % The function ephemeris expects input in binary form. In Matlab 
                ...
                if (firstSubFrame == inf)
                    firstSubFrame = (index(i) * 2 + G1orG2 - 3)*10 + bitSyncPos;
                    TOW = TOW_temp;
                    eph.TOW = TOW;
                end
