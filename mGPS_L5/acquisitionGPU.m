...
%searches for GPS L5 signals of all satellites, which are listed in field
...
% L5c code phases of detected signals
...
for PRN = settings.acqSatelliteList
    ...
    localL5ICode = gpuArray(single([L5ICodesTable, zeros(1,samplesPerCode)]));
    localL5QCode = gpuArray(single([L5QCodesTable, zeros(1,samplesPerCode)]));
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        for nonCohIndex = 1: settings.acqNonCohTime
			...
			results = results + abs(ifft(convL5I)) + abs(ifft(convL5Q));
        end
    ...
    %--- Find 1 chip wide L5 code phase exclude range around the peak ----
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % L5 data and pilot codes
        L5ICode = gpuArray(single(generateL5Icode(PRN,settings)));
        L5QCode = gpuArray(single(generateL5Qcode(PRN,settings)));

        % L5 codes sample index
        codeValueIndex = gpuArray(single(floor( (0 : 200*samplesPerCode -1) ...
            * ts * settings.codeFreqBasis   )));
        
        % Sampled data and pilot codes
        L5ICode200ms = L5ICode((rem(codeValueIndex, settings.codeLength) + 1));
        L5QCode200ms = L5QCode((rem(codeValueIndex, settings.codeLength) + 1));
        ...
        basebandData = reshape(sig200ms .* L5ICode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
        basebandPilot = reshape(sig200ms .* L5QCode200ms, ...
            samplesPerCode, 200) .* localCarr1ms;
