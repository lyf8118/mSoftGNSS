...
% Performs cold-start acquisition for GLONASS L2OC signals using CPU-based
% MATLAB arrays. The function searches all PRNs listed in
% settings.acqSatelliteList and stores the detected L2OCp code phase, L2OCp overlay code
...
% Make index array to read code values
...
for PRN = settings.acqSatelliteList
    ...
    %% Coarse acquisition =========================================
    % Generate all L2OCp codes and sample them according to the sampling freq.
    ...
    %--- Find 1-chip-wide L2 OC code phase exclude range around the peak ---
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % L2OCp codes
        L2CmCode = generateL2OcpBOCCode(PRN);
        
        % Resample the local L2OCp code over 10 L2OCp code periods (200 ms).
