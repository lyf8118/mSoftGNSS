...
% Performs GLONASS L2OC code and carrier tracking in channel-serial mode.
%
...
%                       frequency, code phase,code phase and status.
...
%--- DLL variables --------------------------------------------------------
% Code length of one coherent integration in equivalent L2OC chips.
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        % Get a vector with the L2OC code sampled 1x/chip
        L2OCpCode = generateL2OcpBOCCode(channel(channelNr).PRN);
        % Then make it possible to do early and late versions
        L2OCpCode = [L2OCpCode(codeLength) L2OCpCode L2OCpCode(1)];
		
		L2CSICode = zeros(1, codeLength * fineFactor);
		L2CSICode = [L2CSICode(end) L2CSICode L2CSICode(1)];
		L2OCCodeTable = [L2OCpCode L2CSICode];
        ...
        for loopCnt =  1:NumToProcess
            ...
            if settings.correlatorType == 0 % Using MATLAB correlator
                correValues = corrMatlabSerialTMBPSK(settingsCorr, rawSignal, ...
                    L2OCCodeTable, remCarrPhase, carrPhaseStep,remCodePhaseL2OC * fineFactor,...
