...
% Performs parallel Galileo E1 code and carrier tracking for all enabled
...
% Absolute sample index of each channel's current E1 code epoch.
...
%% Generate the E1 code table for acquired signals =======================
...
blkEndIdx = 0;
% Nominal number of input samples per E1 primary-code period.
samplesPerCode = round(settings.samplingFreq / ...
    (settings.codeFreqBasis / settings.codeLength));
% Read one second of data at a time for the parallel correlator.
samplesPerSec = samplesPerCode * round(settings.codeFreqBasis / settings.codeLength);
...
for loopCnt =  1:NumToProcess
    ...
    % Number of input samples needed to complete the current E1
    ...
    if settings.correlatorType == 0 % Using MATLAB correlator
        ...
        correValues = corrMatlabParallelE1(settings,rawSignal,E1CodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,E1CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase*12,codePhaseStep*12,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        % Uses the same QPSK interface and 12x code phase units.
        correValues = corrGPUParallelQPSK(settings,rawSignal,E1CodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase*12,codePhaseStep*12,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
