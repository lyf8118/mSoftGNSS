...
for channelNr = activeChnList
    ...
    elseif (eph(PRN).E1b_HS ~= 0)
        fprintf('    PRN %02d E1-B signal is unhealthy and is excluded!\n', PRN);
        activeChnList = setdiff(activeChnList, channelNr);
    elseif (eph(PRN).E1B_DVS ~= 0)
        fprintf('    PRN %02d E1-B navigation data are invalid and are excluded!\n', PRN);
        activeChnList = setdiff(activeChnList, channelNr);
    elseif (eph(PRN).SISA == 255)
        fprintf('    PRN %02d reports SISA NAPA and is excluded!\n', PRN);
        activeChnList = setdiff(activeChnList, channelNr);
    else
