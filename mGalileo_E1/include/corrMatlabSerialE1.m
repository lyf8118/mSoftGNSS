function correValues = corrMatlabSerialE1(settings, rawSignal, E1CodeTable, ...
    remCarrPhase, carrPhaseStep, remCodePhase, codePhaseStep)

codeLen = length(E1CodeTable)/2;
E1BDataCode = E1CodeTable(1:codeLen);
E1CPilotCode = E1CodeTable(1+codeLen:end);
...
tcode2      = ceil(tcode) + 1;
earlyCode   = E1BDataCode(tcode2);
% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    pilotEarlyCode   = E1CPilotCode(tcode2);
end
...
tcode       = (remCodePhase+earlyLateSpc) : ...
    ...
    ((blksize-1)*codePhaseStep+remCodePhase+earlyLateSpc);
tcode2      = ceil(tcode) + 1;
lateCode    = E1BDataCode(tcode2);
% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    pilotLateCode = E1CPilotCode(tcode2);
end
...
tcode2      = ceil(tcode) + 1;
promptCode  = E1BDataCode(tcode2);
% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    pilotPromptCode = E1CPilotCode(tcode2);
end
