...
%searches for GLONASS L3OC signals of all satellites, which are listed in field
...
% L3Oc code phases of detected signals
...
for PRN = settings.acqSatelliteList 
    ...
    % Generate all L3Ocd+L3Ocp primary codes and sample them
    L3OcdCodesTable = makeL3OcdTable(settings,PRN); 
    L3OcpCodesTable = makeL3OcpTable(settings,PRN);
    % generate local code duplicate to do correlate
    L3OcdCode2ms = gpuArray(single([L3OcdCodesTable, zeros(1,samplesPerCode)])); 
    L3OcpCode2ms = gpuArray(single([L3OcpCodesTable, zeros(1,samplesPerCode)]));
    
    %--- Perform DFT of PRN code ------------------------------------------
    L3OcdCodeFreqDom = conj(fft(L3OcdCode2ms)); 
    L3OcpCodeFreqDom = conj(fft(L3OcpCode2ms));
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        for nonCohIndex = 1: settings.acqNonCohTime 
            ...
            results = results + abs(ifft(convL3Ocd))+abs(ifft(convL3Ocp)); 
        end
    ...
    %--- Find 1 chip wide L3OC code phase exclude range around the peak ----
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % L3Oc data and pilot codes
         L3OcdCode = gpuArray(single(generateL3OcdCode(PRN))); 
         L3OcpCode = gpuArray(single(generateL3OcpCode(PRN)));

        % L3Oc codes sample index
        codeValueIndex = gpuArray(single(floor( (0 : 200*samplesPerCode -1) ...
            * ts * settings.codeFreqBasis   )));

        % Sampled data and pilot codes
         L3OcdCode200ms = L3OcdCode((rem(codeValueIndex, settings.codeLength) + 1));
         L3OcpCode200ms = L3OcpCode((rem(codeValueIndex, settings.codeLength) + 1));
        ...
        % Wipe off code and carrier from incoming signals
         basebandData = reshape(sig200ms .* L3OcdCode200ms, ...
             samplesPerCode, 200) .* localCarr1ms;
         basebandPilot = reshape(sig200ms .* L3OcpCode200ms, ...
             samplesPerCode, 200) .* localCarr1ms;
