function correValues = corrMatlabSerialQPSK(settings, rawSignal, L3OCCodeTable, ...
    remCarrPhase, carrPhaseStep, remCodePhase, codePhaseStep)

codeLen = length(L3OCCodeTable)/2;
L3OCCodeD = L3OCCodeTable(1:codeLen);
L3OCCodeP = L3OCCodeTable(1+codeLen:end);
...
tcode       = (remCodePhase+earlyLateSpc) : ...
    ...
    ((blksize-1)*codePhaseStep+remCodePhase+earlyLateSpc);
tcode2      = ceil(tcode) + 1;
lateCode    = L3OCCodeD(tcode2);

% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    lateCodeQ   = L3OCCodeP(tcode2);
end
...
tcode2      = ceil(tcode) + 1;
promptCode  = L3OCCodeD(tcode2);

% For pilot channel signal tracking
if (settings.pilotTRKflag == 1)
    promptCodeQ   = L3OCCodeP(tcode2);
end
