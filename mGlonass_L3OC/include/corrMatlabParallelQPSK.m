function correValues = corrMatlabParallelQPSK(settings, rawSignal, L3OCCodeTable, ...
    remCarrPhase, carrPhaseStep, remCodePhase, codePhaseStep, ...
    startIdx, chSampSize, isDataRead)
%CORRMATLABPARALLELQPSK Matlab implementation of the channel-parallel QPSK tracking correlator.
...
for channelNr = 1:channelCnt
    ...
    codeLen = length(L3OCCode)/2;
    L3OCCodeD = L3OCCode(1:codeLen);
    if (settings.pilotTRKflag == 1)
        L3OCCodeP = L3OCCode(1+codeLen:end);
    end
