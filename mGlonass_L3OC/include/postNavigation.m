...
eph_template = eph_structure_init();
eph = repmat(eph_template, 1, 64);
...
for channelNr = activeChnList
   ...
   [eph_temp, start_temp, tod_temp] = NAVdecoding(trackResults(channelNr).I_P);
    
   % Identify missing fields
   missing_fields = {};
    if isempty(eph_temp.N4), missing_fields{end+1} = 'N4 (Type 10)'; end
    if isempty(eph_temp.X),  missing_fields{end+1} = 'X (Type 11)'; end
    
    if ~isempty(missing_fields)
        activeChnList = setdiff(activeChnList, channelNr);
        fprintf('    [x] Ephemeris incomplete for PRN %d. Missing: %s\n', ...
                K, strjoin(missing_fields, ', '));
        continue; 
    end

    if ~isempty(eph_temp.X)
        eph(K) = eph_temp;
    end
    subFrameStart(channelNr) = start_temp;
    TOD(channelNr) = tod_temp;
    ...
    %elseif isfield(eph_temp, 'Health') == 1
        %activeChnList = setdiff(activeChnList, channelNr);
        ...
        if ~isempty(eph(channelNr).Tau_c)
...
if (isempty(activeChnList) || (size(activeChnList, 2) < 3))
...
% Second term is to make space to aviod index exceed matrix dimensions, 
...
for currMeasNr = 1:measNrSum
    ...
    txTimeList = transmitTime(activeChnList);
    
    % Save satClkCorr
    [satPositions, satClkCorr] = satpos(txTimeList, prnList, eph);
