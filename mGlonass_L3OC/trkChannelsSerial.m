...
% Performs code and carrier tracking for L3OC signals of all channels.
%
...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        L3OcdCode = [L3OcdCode(settings.codeLength) L3OcdCode L3OcdCode(1)]; %#ok<AGROW>

            % Get a vector with the L3OC pilot code sampled 1x/chip
            L3Ocpcode = generateL3OcpCode(channel(channelNr).PRN);
			% Then make it possible to do early and late versions
            L3Ocpcode = [L3Ocpcode(end) L3Ocpcode L3Ocpcode(1)];
        L3OCCodeTable = [L3OcdCode L3Ocpcode];
        ...
        for loopCnt =  1:codePeriods
            ...
            % chaannel signals
            if (settings.pilotTRKflag == 1)
                % Pilot channel(L3OCp) carrier phase is pi/2 rad ahead of the
                % data channel (L3OCd) carrier phase. Here we rotate the pilot
