...
% The absolute sample in the record of the L3OC code start:
...
if (settings.pilotTRKflag == 1)
    ...
    trackResults.L3OC_CNo  = zeros(1,floor(settings.msToProcess/settings.CNoInterval));
end
...
% The absolute sample in the record of the L3OC code start for all channels.
...
% Generate the L3OC code table for acquired signals
for channelNr = 1:channelCnt  
    % Get a vector with the L3OC data code sampled 1x/chip
    L3OcdCode = generateL3OcdCode(channel(channelNr).PRN); 
    % Then make it possible to do early and late versions
    L3OCdCodeTable(channelNr,:)  = [L3OcdCode(end) L3OcdCode L3OcdCode(1)]; %#ok<AGROW>  
    % Get a vector with the L3OC pilot code sampled 1x/chip
    L3OcpCode = generateL3OcpCode(channel(channelNr).PRN);  
    % Then make it possible to do early and late versions
    L3OCpCodeTable(channelNr,:)  = [L3OcpCode(end) L3OcpCode L3OcpCode(1)]; %#ok<AGROW>
end
L3OCCodeTable = [L3OCdCodeTable L3OCpCodeTable];   
if settings.correlatorType == 1
    % SIMD backend expects one int32 code table shared by all active channels.
    L3OCCodeTable = int32(L3OCCodeTable');  
elseif settings.correlatorType == 2
    % GPU backend expects one int8 code table shared by all active channels.
    L3OCCodeTable = int8(L3OCCodeTable');
end

%% Initialize tracking variables ==================================
% Code periods to be processed
codePeriods = settings.msToProcess;    % For GLONASS one L3OC code is one ms
%--- DLL variables --------------------------------------------------------
% Summation interval
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelQPSK(settings,rawSignal,L3OCCodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelQPSK(settings,rawSignal,L3OCCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelQPSK(settings,rawSignal,L3OCCodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
    ...
    for channelNr = 1:channelCnt
        ...
        % chaannel signals
        if (settings.pilotTRKflag == 1)
        % L3OC pilot channel carrier phase is pi/2 rad ahead of the
        ...
        if (rem(loopCnt,settings.CNoInterval)==0) 
            ...
            if (settings.pilotTRKflag == 1)
                ...
                trackResults(channelNr).L3OC_CNo(CNoCnt) = ... 
