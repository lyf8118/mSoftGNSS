...
% The absolute sample in the record of the B2b code start:
...
% The absolute sample in the record of the B2b code start for all channels.
...
% Generate the B2b code table for acquired signals
for channelNr = 1:channelCnt
    % Get a vector with the B2bI data code sampled 1x/chip
    B2bICode = generateB2bICode(channel(channelNr).PRN,settings);
    % Then make it possible to do early and late versions
    B2bICodeTable(channelNr,:) = [B2bICode(end) B2bICode B2bICode(1)];  %#ok<AGROW>
end

if settings.correlatorType == 1
    % For SIMD formatting
    B2bICodeTable = int32(B2bICodeTable');
elseif settings.correlatorType == 2
    % For GPU formatting
    B2bICodeTable = int8(B2bICodeTable');
end
%% Initialize tracking variables ==================================
% Code periods to be processed
codePeriods = settings.msToProcess;     % For BDS one B2bI code is one ms
%--- DLL variables --------------------------------------------------------
% Summation interval
...
for loopCnt =  1:codePeriods
    ...
    if settings.correlatorType == 0 % Using Matlab correlator
        correValues = corrMatlabParallelBPSK(settings,rawSignal,B2bICodeTable, ...
    ...
    elseif settings.correlatorType == 1 % Using SIMD of CPU
        ...
        correValues = corrSIMDParallelBPSK(settings,rawSignal,B2bICodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
    ...
    elseif settings.correlatorType == 2 % Using GPU
        ...
        correValues = corrGPUParallelFusedBPSK(settings,rawSignal,B2bICodeTable, ...
            remCarrPhase,carrPhaseStep,remCodePhase,codePhaseStep,...
            int32(startIdx),int32(chSampSize),isDataRead);
    end
...
% Close the waitbar
close(hwb)
clear corrSIMDParallelBPSK corrGPUParallelFusedBPSK;
