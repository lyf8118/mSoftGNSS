...
%--------------------------------------------------------------------------
%                         CU Multi-GNSS SDR
% (C) Developed for BDS B2b SDR by Yafeng Li, Nagaraj C. Shivaramaiah 
% and Dennis M. Akos. 
...
% It is necessary to have at least two messages (type 10 and 30) 
% to find satellite coordinates. Then receiver 
% position can be found. The function requires at least 8 message.
% One message length is 1 seconds, therefore we need at least 18 sec long
% record (2 * 8 = 16 sec = 16000ms).
...
for channelNr = activeChnList
    ...
    fprintf('Decoding B-CNAV3 for PRN %02d of B2bI signals -------------------- \n', PRN);

    %=== Decode ephemerides and TOW of the first sub-frame ================
    [eph(PRN), subFrameStart(channelNr), TOW(channelNr)] = ...
        BCNAV3decoding(trackResults(channelNr).I_P,...
        trackResults(channelNr).Q_P,PRN);  
    ...
    if (eph(PRN).idValid(1) ~= 10 || eph(PRN).idValid(2) ~= 30 )
        ...
        if (eph(PRN).idValid(3) ~= 40)
            fprintf('  Message type 40 for PRN %02d not decoded.\n', PRN);  
        end                
        fprintf('  Channel for PRN %02d excluded!!\n', PRN);
    elseif (eph(PRN).HS ~= 0)
        activeChnList = setdiff(activeChnList, channelNr);
        fprintf('    PRN %02d is unhealthy and is excluded!\n', PRN);
    else       
        fprintf('  Requisite messages for PRN %02d all decoded!\n', PRN);
    end 
