...
% B2BI code phases of detected signals
...
for PRN = settings.acqSatelliteList
    ...
    B2bITable  = makeB2bITable(PRN,settings);
    % Add zero-padding samples
    B2bICodes2ms = gpuArray(single([B2bITable zeros(1,samplesPerCode)]));
    
    %--- Perform DFT of B2b code ------------------------------------------
    B2bIFreqDom = conj(fft(B2bICodes2ms));
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        for nonCohIndex = 1: settings.acqNonCohTime
            ...
            % time domain)
            convCodeIQ = IQfreqDom .* B2bIFreqDom;
    ...
    %--- Find 1 chip wide B2b code phase exclude range around the peak ----
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % B2b code with 1023 chips
        B2bICode = gpuArray(single(generateB2bICode(PRN,settings)));
        
        % B2bI code sample index
        codeValueIndex = gpuArray(single(floor((ts * ...
            (0 : 200*samplesPerCode -1)) / (1/settings.codeFreqBasis))));
        
        % B2bI code samples
        B2bICode200ms = B2bICode(rem(codeValueIndex, settings.codeLength) + 1);
        ...
        basebandSig = reshape(sig200ms .* B2bICode200ms, samplesPerCode, 200) ...
            .* localCarr1ms;
