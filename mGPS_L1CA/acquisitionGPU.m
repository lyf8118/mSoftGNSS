...
%searches for GPS L1 C/A signals of all satellites, which are listed in field
...
for PRN = settings.acqSatelliteList
    ...
    caCodes2ms = gpuArray(single(caCodes2ms));
    ...
    if acqResults.peakMetric(PRN) > settings.acqThreshold
        ...
        % C/A code with 1023 chips
        caCode = gpuArray(single(generateCAcode(PRN)));
        % C/A code sample index
        codeValueIndex = gpuArray(single(floor( (0 : 200*samplesPerCode -1) ...
            * ts * settings.codeFreqBasis   )));
        % C/A code samples
        caCode200ms = caCode(rem(codeValueIndex, settings.codeLength) + 1);
        ...
        basebandSig = reshape(sig200ms .* caCode200ms, samplesPerCode, 200) ...
            .* localCarr1ms;
