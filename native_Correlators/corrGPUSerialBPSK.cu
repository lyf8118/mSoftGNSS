 ...
 * Description: GPU-assisted single-channel tracking correlator for
 *              trkChannelsSerial.m.
 *
 ...
 *          plhs[0]   double*     corrValues     O   [IE QE IP QP IL QL]
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (!initialized)
    {
         ...
         * codeLen      : number of code chips in the local code table
    ...
    plhs[0] = mxCreateDoubleMatrix((mwSize)1, (mwSize)6, mxREAL);
    ...
    correlator<<<CORR_NUMBER, THREADS_PER_BLOCK>>>(
        ...
        (int)codeLen, (float)codePhaseStep, initCodePhase, blksize);
    CUDA_KERNEL_CHECK();
