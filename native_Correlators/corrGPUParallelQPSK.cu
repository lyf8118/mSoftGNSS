 ...
 *                                              arranged as [dataCodeTable; pilotCodeTable]
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
	...
	if (!initialized)
	{
         ...
         *------------------------------------------------ */
		// Total concatenated code-table length: [dataCodeTable; pilotCodeTable]
	...
	for (int chInd = 0; chInd < channelCnt; chInd++)
	{
		double remCodePhaseWrapped = fmod(remCodePhase[chInd], (double)halfCodeLen);
	...
	// [I_E, Q_E, I_P, Q_P, I_L, Q_L, pilot_I_E, pilot_Q_E, pilot_I_P, pilot_Q_P, pilot_I_L, pilot_Q_L]^T.
	for (int chInd = 0; chInd < channelCnt; chInd++) {
